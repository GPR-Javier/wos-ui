...
function Onboarding() {
  ...
  const includedKeys = new Set(
    cmp.plans.find((p) => p.slug === slug)?.included ?? []
  )
  const includedCount = cmp.features.filter((f) =>
    includedKeys.has(f.key)
  ).length
      ...
      <PublicHeader
        right={
          <Button asChild size="sm" variant="ghost">
            <Link href={slugHref("/pricing")}>Pricing</Link>
          </Button>
        }
      />
      ...
      <main className="mx-auto max-w-4xl px-4 pt-12 pb-24">
        {!created ? (
          <>
            <div className="text-center">
              ...
              <p className="mt-2 text-[14px] text-muted-foreground">
                ...
                plan{seatNote} and become its admin.
              </p>
            </div>
          ...
          <div className="mx-auto max-w-md text-center">
            ...
            <p className="mt-2 text-[14px] text-muted-foreground">
              Your workspace is set up on the {planName} plan. Sign in as the
              company admin to get started.
            </p>
  ...
  return (
    <Card className="h-fit p-5">
      ...
      <p className="mt-1 text-[11.5px] text-muted-foreground">
        Config modules your workspace can turn on. Locked ones need a higher
        plan.
      </p>
              ...
              <span
                className={
                  on
                    ...
                    : "text-[12.5px] text-muted-foreground line-through decoration-muted-foreground/40"
                }
              >
                {f.label}
              </span>
