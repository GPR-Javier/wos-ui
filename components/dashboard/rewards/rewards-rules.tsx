...
import { Add01Icon, Settings02Icon, StarIcon, GiftIcon } from "@hugeicons/core-free-icons"
import { useRewardRules, useCreateRule, useUpdateRule, useToggleRule } from "@/hooks/use-rewards"
import { type RewardRule, type RewardRulePayload, type RuleType, type RewardType, type TriggerType } from "@/lib/rewards-api"
...
function ruleTypeLabel(t: RuleType) {
  return t === "rating" ? "Rating-Based" : t === "performance" ? "Performance-Based" : "Material"
}

function triggerLabel(t: TriggerType) {
  return t === "per-session" ? "Per session" : t === "weekly-bonus" ? "Weekly bonus" : "Monthly bonus"
}
...
function RuleDialog({
  ...
  function set<K extends keyof RewardRulePayload>(k: K, v: RewardRulePayload[K]) {
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 text-[13px]">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.ruleType} onValueChange={(v) => set("ruleType", v as RuleType)}>
              <SelectTrigger className="text-[12px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="rating" className="text-[12px]">Rating-Based (per star rating)</SelectItem>
                <SelectItem value="performance" className="text-[12px]">Performance-Based (sessions / attendance)</SelectItem>
                <SelectItem value="material" className="text-[12px]">Material Reward (custom item)</SelectItem>
              </SelectContent>
            </Select>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => set("ratingMax", e.target.value ? Number(e.target.value) : null)}
                />
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => set("minSessions", e.target.value ? Number(e.target.value) : null)}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => set("minAttendanceRate", e.target.value ? Number(e.target.value) : null)}
                />
              </div>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.triggerType} onValueChange={(v) => set("triggerType", v as TriggerType)}>
              ...
              <SelectContent>
                <SelectItem value="per-session" className="text-[12px]">Per session</SelectItem>
                <SelectItem value="weekly-bonus" className="text-[12px]">Weekly bonus</SelectItem>
                <SelectItem value="monthly-bonus" className="text-[12px]">Monthly bonus</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.rewardType} onValueChange={(v) => set("rewardType", v as RewardType)}>
              ...
              <SelectContent>
                <SelectItem value="monetary" className="text-[12px]">💰 Monetary (₱ amount)</SelectItem>
                <SelectItem value="material" className="text-[12px]">📦 Material (item / package)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {form.rewardType === "monetary" ? (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => set("monetaryAmount", e.target.value ? Number(e.target.value) : null)}
              />
            </div>
          ) : (
            <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label className="text-[12px]">Effective to <span className="text-muted-foreground">(optional)</span></Label>
              <Input
                type="date"
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Rule will be applied to eligible teachers</p>
            </div>
            <Switch
              checked={form.active}
...
export function RewardsRules() {
  ...
  function openEdit(rule: RewardRule) { setEditing(rule); setDialogOpen(true) }

  const COLUMNS = ["Rule", "Type", "Condition", "Reward", "Trigger", "Dates", "Active", ""]
      ...
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={COLUMNS.length} className="py-8 text-center text-[13px] text-muted-foreground">No rules yet — create one to start computing rewards</TableCell>
            </TableRow>
          ) : (
            rules.map((rule) => (
              <TableRow key={rule.id}>
                ...
                <TableCell>
                  <StatusBadge
                    variant={rule.ruleType === "rating" ? "amber" : rule.ruleType === "performance" ? "blue" : "purple"}
                    dot={false}
                  >
                ...
                <TableCell className="text-[12px] text-muted-foreground">
                  ...
                  {rule.ruleType === "performance" && (
                    <span>
                      {rule.minSessions != null && `≥${rule.minSessions} sessions`}
                      {rule.minAttendanceRate != null && ` / ${rule.minAttendanceRate}% attendance`}
                    </span>
                  )}
                ...
                <TableCell className="text-[12px]">
                  {rule.rewardType === "monetary" ? (
                    <span className="text-success font-medium">₱{rule.monetaryAmount?.toLocaleString("en-PH")}</span>
                  ) : (
                    <span className="flex items-center gap-1">
                      <HugeiconsIcon icon={GiftIcon} size={12} strokeWidth={1.8} className="text-violet-500" />
                      {rule.materialItem}
                    </span>
                  )}
                </TableCell>
                <TableCell className="text-[12px] text-muted-foreground">{triggerLabel(rule.triggerType)}</TableCell>
                ...
                <TableCell>
                  <Button size="xs" variant="ghost" onClick={() => openEdit(rule)}>
