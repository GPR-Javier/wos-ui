...
const EMPTY_FORM = {
  ...
  travelMode: "DOMESTIC" as TravelMode,
  transportType: "" as TransportType | "",
  budgetTransportation: "" as string,
  budgetAccommodation: "" as string,
  budgetMeals: "" as string,
  budgetMiscellaneous: "" as string,
  accommodation: "",
  remarks: "",
  files: [] as File[],
}
...
function RequestFormDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function totalBudget() {
    return (
      Number(form.budgetTransportation || 0) +
      Number(form.budgetAccommodation || 0) +
      Number(form.budgetMeals || 0) +
      Number(form.budgetMiscellaneous || 0)
    )
  }
  ...
  function submit(isDraft: boolean) {
    if (!form.purpose || !form.transportType || !form.destination) return
    const budgetAllocation = {
      transportation: Number(form.budgetTransportation || 0),
      accommodation: Number(form.budgetAccommodation || 0),
      meals: Number(form.budgetMeals || 0),
      miscellaneous: Number(form.budgetMiscellaneous || 0),
    }
    create.mutate(
      {
        ...
        purpose: form.purpose as TripPurpose,
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          {/* Travel Mode */}
          <div className="space-y-1.5">
            <Label className="text-[12px]">Travel Mode</Label>
            <div className="grid grid-cols-2 gap-2">
              {(["DOMESTIC", "INTERNATIONAL"] as TravelMode[]).map((m) => (
                <button
                  key={m}
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, travelMode: m }))}
                  className={cn(
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(
                [
                  { key: "budgetTransportation", label: "Transportation" },
                  { key: "budgetAccommodation", label: "Accommodation" },
                  { key: "budgetMeals", label: "Meals & Per Diem" },
                  { key: "budgetMiscellaneous", label: "Miscellaneous" },
                ] as const
              ).map(({ key, label }) => (
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/20 px-3 py-2 text-[12px] text-muted-foreground">
        ...
        <DialogFooter className="gap-2">
          {canCancel && (
            <Button
              ...
              onClick={() => cancel.mutate(trip.id, { onSuccess: onClose })}
            >
...
export function MyBusinessTripSection() {
  ...
  const q = useMyBusinessTrips({ status: statusFilter, page, size: 20 })
  const items = q.data?.content ?? []
  const total = q.data?.totalElements ?? 0
  const totalPages = q.data?.totalPages ?? 0

  const summaryQ = useMyBusinessTrips({ size: 200 })
