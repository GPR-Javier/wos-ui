...
import { useEmployeeStats, useAttendance, useLeaveBalances } from "@/hooks/use-employee"
    ...
    <div className="flex items-end gap-0.5 h-9">
      {data.map((v, i) => (
        <div
          ...
          style={{ height: `${Math.max((v / max) * 100, v > 0 ? 8 : 3)}%`, opacity: v > 0 ? 0.85 : 0.2 }}
        />
      ))}
    </div>
  )
...
const NOTIFICATIONS = [
  { text: "Your schedule for next week has been updated.", type: "info" as const, time: "2h ago" },
  { text: "Leave request approved — Dec 25–26.", type: "success" as const, time: "Yesterday" },
  { text: "Reminder: You have a missing time-out log from Monday.", type: "warn" as const, time: "2d ago" },
  { text: "Upcoming shift tomorrow: 8:00 AM – 5:00 PM.", type: "info" as const, time: "3d ago" },
]

const REQUEST_TYPES = [
  { label: "Leave Requests", section: "request", color: "bg-blue-500" },
  { label: "Attendance Corrections", section: "my-change-time", color: "bg-amber-500" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <StatCard
          ...
          value={
            todayEntry ? (
              <StatusBadge variant={STATUS_VARIANT[todayEntry.status] ?? "gray"} className="text-sm font-bold">
                {STATUS_LABEL[todayEntry.status] ?? todayEntry.status}
              </StatusBadge>
            ) : statsQ.isLoading ? <Skeleton className="h-6 w-16" /> : (
              <span className="text-muted-foreground text-sm">—</span>
            )
          }
          meta="Current attendance"
          accent={todayEntry ? (STATUS_VARIANT[todayEntry.status] === "green" ? "green" : STATUS_VARIANT[todayEntry.status] === "amber" ? "amber" : "red") : "gray"}
          icon={<HugeiconsIcon icon={CheckmarkCircle02Icon} size={15} strokeWidth={1.8} />}
        />
        <StatCard
          title="Worked Hours"
          value={todayEntry?.hoursWorked ?? (attendanceQ.isLoading ? <Skeleton className="h-6 w-12" /> : "—")}
          meta="Today"
          accent="green"
          icon={<HugeiconsIcon icon={Clock01Icon} size={15} strokeWidth={1.8} />}
        />
        <StatCard
          title="Late Count"
          value={stats ? String(stats.lateIncidents) : (statsQ.isLoading ? <Skeleton className="h-6 w-8" /> : "—")}
          meta="This year"
          accent={stats && stats.lateIncidents > 0 ? "amber" : "green"}
          icon={<HugeiconsIcon icon={Alert01Icon} size={15} strokeWidth={1.8} />}
        />
        <StatCard
          title="OT Hours"
          value={todayEntry?.otHours ?? (attendanceQ.isLoading ? <Skeleton className="h-6 w-12" /> : "0h")}
          meta="Today"
          accent="purple"
          icon={<HugeiconsIcon icon={UserCircleIcon} size={15} strokeWidth={1.8} />}
        />
        <StatCard
          ...
          accent="red"
          icon={<HugeiconsIcon icon={Calendar01Icon} size={15} strokeWidth={1.8} />}
        />
        <StatCard
          title="Next Shift"
          value={<span className="text-sm font-semibold tabular-nums">{shiftLabel}</span>}
          meta={policy ? `${policy.requiredHours ?? "—"}h required` : "Schedule policy"}
          accent="blue"
          icon={<HugeiconsIcon icon={Clock01Icon} size={15} strokeWidth={1.8} />}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="mb-3 text-[12px] font-semibold text-foreground">Today's Attendance</p>
          <ClockWidget />
          {todayEntry && (
        ...
        {/* My Schedule */}
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="mb-3 text-[12px] font-semibold text-foreground">My Schedule</p>
          <div className="space-y-2.5">
            {[
              {
                label: "Today's Shift",
                value: policy ? `${policy.earliestClockIn} – ${policy.latestClockIn}` : "—",
                sub: policy ? `${policy.requiredHours}h · Grace ${policy.lateGraceMins ?? 0}m` : "",
                dot: "bg-green-500",
              },
              {
              ...
              {balances.slice(0, 3).map((l) => (
                <div key={l.type}>
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-semibold">{l.remaining}/{l.total} days</span>
                  </div>
                  <Progress
      ...
      {/* ── 3. Requests Area ─────────────────────────────────── */}
      <div>
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">My Requests</p>
          ...
          {REQUEST_TYPES.map(({ label, section, color }) => (
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                <HugeiconsIcon icon={ArrowRight01Icon} size={12} strokeWidth={2} className="text-muted-foreground/40 transition-colors group-hover:text-primary" />
              </div>
      ...
      <div>
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Personal Analytics</p>
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div className="rounded-xl border border-border bg-card p-4">
            <p className="text-[12px] font-semibold text-foreground">Attendance Rate</p>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <p className="text-[12px] font-semibold text-foreground">Monthly Hours</p>
            <p className="mb-3 text-[11px] text-muted-foreground">Hours worked</p>
            ...
            {stats && (
              <p className="mt-2 text-[11px] text-muted-foreground">
                YTD: <span className="font-semibold text-foreground">{stats.totalHoursWorked.toLocaleString()}h</span>
              </p>
            )}
          </div>

          <div className="rounded-xl border border-border bg-card p-4">
            <p className="text-[12px] font-semibold text-foreground">Late Trend</p>
            <p className="mb-3 text-[11px] text-muted-foreground">Minutes late per day</p>
            ...
            {stats && (
              <p className="mt-2 text-[11px] text-muted-foreground">
                {stats.lateIncidents} incident{stats.lateIncidents !== 1 ? "s" : ""} this year
              </p>
            )}
          </div>

          <div className="rounded-xl border border-border bg-card p-4">
            <p className="text-[12px] font-semibold text-foreground">OT Summary</p>
            <p className="mb-3 text-[11px] text-muted-foreground">OT hours per day</p>
          ...
          {NOTIFICATIONS.map((n, i) => (
            <div
              ...
              className={cn(
                ...
                n.type === "warn" && "border-amber-200 bg-amber-50 dark:border-amber-900/40 dark:bg-amber-900/10",
                n.type === "success" && "border-green-200 bg-green-50 dark:border-green-900/40 dark:bg-green-900/10",
