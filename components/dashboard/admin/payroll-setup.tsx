...
const COMPENSATION_BASIS_OPTIONS: {
...
const DEDUCTION_SCHEDULE_OPTIONS: {
...
}[] = [
  {
    value: "EVERY_PAYROLL",
    label: "Every payroll",
    sub: "Every cutoff period",
  },
  {
    value: "FIRST_PAYROLL",
    label: "1st payroll only",
    sub: "First cutoff of the month",
  },
  {
    value: "SECOND_PAYROLL",
    label: "2nd payroll only",
    sub: "Second cutoff of the month",
  },
...
type FormErrors = Partial<
  Record<keyof Omit<SetupForm, "allowances" | "deductions">, string>
>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="mb-3 text-[11px] font-semibold tracking-wide text-muted-foreground/70 uppercase">
      {children}
    </p>
  )
}
...
function AllowancesEditor({
  ...
  items: AllowanceDraft[]
  onChange: (v: AllowanceDraft[]) => void
  symbol: string
}) {
  function add() {
    onChange([...items, { name: "", amount: "" }])
  }
  ...
  return (
    <div className="space-y-2">
      {items.length === 0 ? (
        <p className="rounded-lg border border-dashed py-4 text-center text-[12px] text-muted-foreground italic">
          No allowances added
        </p>
      ) : (
        items.map((item, idx) => (
          <div key={idx} className="flex min-w-0 items-center gap-2">
            ...
            <div className="relative w-32 shrink-0">
              <span className="absolute top-1/2 left-2.5 -translate-y-1/2 text-[12px] text-muted-foreground">
                {symbol}
              </span>
              <Input
                type="number"
...
function DeductionsEditor({
  ...
  items: DeductionDraft[]
  onChange: (v: DeductionDraft[]) => void
  symbol: string
}) {
  function add() {
    onChange([
  ...
  function updatePreset(idx: number, preset: string) {
    onChange(
      items.map((item, i) =>
        i === idx
          ? { ...item, preset, name: preset === "__custom__" ? "" : preset }
          : item
      )
    )
  }
    ...
    onChange(
      items.map((item, i) =>
        i === idx
          ? {
              ...item,
              amountType: item.amountType === "FIXED" ? "PERCENTAGE" : "FIXED",
              amount: "",
            }
          : item
      )
        ...
        items.map((item, idx) => (
          ...
          >
            <div className="flex min-w-0 items-center gap-2">
              <Select
                value={item.preset}
                onValueChange={(v) => updatePreset(idx, v)}
              >
                <SelectTrigger
                  className={cn(
                ...
                <SelectContent>
                  {["Government", "Loans"].map((group) => (
                    <div key={group}>
                      ...
                      {DEDUCTION_PRESETS.filter((p) => p.group === group).map(
                        (p) => (
                          <SelectItem
                            key={p.value}
                            value={p.value}
            ...
            {item.preset && (
              <div className="flex items-center gap-2 rounded-md bg-background/60 px-2.5 py-1.5">
                ...
                <Select
                  value={item.schedule}
                  onValueChange={(v) => update(idx, "schedule", v)}
                >
...
function SetupModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Dialog */}
      <div
        className="relative flex w-full max-w-3xl animate-in flex-col rounded-2xl border border-border bg-card shadow-xl duration-200 zoom-in-95 fade-in"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FieldWrap label="Job Position *" error={errors.jobPositionId}>
                <Select
                  ...
                  onValueChange={(v) => setField("jobPositionId", v)}
                >
                  <SelectTrigger
                    className={cn(
                      "h-9 text-[13px]",
                      errors.jobPositionId && "border-destructive"
                    )}
                  >
                  ...
                  <SelectContent className="max-h-60">
                    {positions
                      .filter(
                        (p) => p.active || String(p.id) === form.jobPositionId
                      )
                      .map((p) => (
                        <SelectItem
                          key={p.id}
          ...
          <FormCard>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FieldWrap
                ...
                error={errors.compensationBasis}
              >
                <Select
                  value={form.compensationBasis}
                  onValueChange={(v) =>
                    setField("compensationBasis", v as CompensationBasis)
                  }
                >
              ...
              <FieldWrap label="Cutoff Period *" error={errors.cutoffPeriod}>
                <Select
                  value={form.cutoffPeriod}
                  onValueChange={(v) => setField("cutoffPeriod", v)}
                >
                  <SelectTrigger
                    className={cn(
                      "h-9 text-[13px]",
                      errors.cutoffPeriod && "border-destructive"
                    )}
                  >
            ...
            <div className="flex flex-wrap items-center gap-4">
              <label className="flex cursor-pointer items-center gap-2.5">
                <button
                  type="button"
                  role="switch"
                  aria-checked={form.overtimeEligible}
                  onClick={() =>
                    setField("overtimeEligible", !form.overtimeEligible)
                  }
                  className={cn(
                ...
                <FieldWrap label="Rate multiplier" error={errors.overtimeRate}>
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      className={cn(
                        "h-9 w-28 text-[13px]",
                        errors.overtimeRate && "border-destructive"
                      )}
                      placeholder="1.25"
...
export function PayrollSetupSection() {
  ...
  function validate(): boolean {
    ...
    if (
      form.overtimeEligible &&
      (!form.overtimeRate || isNaN(parseFloat(form.overtimeRate)))
    )
    ...
    if (editingId) {
      updateMut.mutate(
        { id: editingId, payload },
        {
          onSuccess: () => {
            closeForm()
            showToast("Payroll setup updated.", "success")
          },
  ...
  function handleDelete(s: PayrollSetup) {
    if (
      !confirm(
        `Delete payroll setup for "${s.jobPositionTitle}"? This cannot be undone.`
      )
    )
      return
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[12px] text-muted-foreground">
            Link job positions to salary grades and define compensation rules,
            cutoff periods, overtime, allowances, and deductions.
          </p>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No payroll setups yet"
          description="Create a setup to define how each job position is compensated."
          action={
            <Button size="sm" onClick={openCreate}>
              <HugeiconsIcon
                icon={Add01Icon}
        ...
        <EmptyState title="No setups match your filters." />
      ) : (
        <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-md bg-muted px-1.5 py-0.5 text-[11px] font-medium">
                        {basisLabel(s.compensationBasis)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {s.deductions.length > 0 ? (
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          {s.deductions.some(
                            (d) => d.schedule && d.schedule !== "EVERY_PAYROLL"
                          ) && (
                            <span className="text-[10px] text-muted-foreground">
                              mixed
                            </span>
          ...
          <div className="border-t px-4 py-2.5 text-right text-[11px] text-muted-foreground">
            {filtered.length} of {setups.length} setup
            {setups.length !== 1 ? "s" : ""}
          </div>
        </div>
      )}
        ...
        (() => {
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              />
              <div
                className="relative flex w-full max-w-lg animate-in flex-col rounded-2xl border border-border bg-card shadow-xl duration-200 zoom-in-95 fade-in"
                  ...
                  <div>
                    ...
                    <div className="divide-y divide-border/60 rounded-lg border border-border/60">
                      <div className="flex justify-between px-3 py-2">
                        ...
                        <span className="font-medium">
                          {basisLabel(viewingItem.compensationBasis)}
                        </span>
                      </div>
                      <div className="flex justify-between px-3 py-2">
                        <span className="text-muted-foreground">
                        ...
                        <span className="font-medium">
                          {viewingItem.cutoffPeriod}
                        </span>
                      </div>
                      <div className="flex justify-between px-3 py-2">
                        <span className="text-muted-foreground">Overtime</span>
                        <span className="font-medium">
                          {viewingItem.overtimeEligible
                            ? `Eligible · ${viewingItem.overtimeRate}× rate`
                            : "Not eligible"}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="divide-y divide-border/60 rounded-lg border border-border/60">
                        {viewingItem.deductions.map((d, i) => (
                          ...
                          >
                            ...
                            <span className="font-mono font-medium">
                              {d.amountType === "PERCENTAGE"
                                ? `${d.amount}%`
                                : `${sym}${d.amount.toLocaleString("en-PH")}`}
                            </span>
                          </div>
                        ))}
                      </div>
