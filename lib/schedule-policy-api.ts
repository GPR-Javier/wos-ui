...
export const schedulePolicyApi = {
  getCurrent: (scope: PolicyScope, scopeRef?: number | null) =>
    api
      .get<PolicyVersion | "">("/hr/schedule-policies/" + scope, {
  ...
  save: (scope: PolicyScope, body: SavePolicyRequest) =>
    api
      .post<PolicyVersion>("/hr/schedule-policies/" + scope, body)
      .then((r) => r.data),
  ...
  resolveForUser: (userId: number) =>
    api
      .get<ResolvedPolicy>(`/hr/schedule-policies/resolve/user/${userId}`)
      .then((r) => r.data),
  ...
  listOverrides: (scope: PolicyScope) =>
    api
      .get<PolicyVersion[]>(`/hr/schedule-policies/${scope}/overrides`)
      .then((r) => r.data),
  ...
  reset: (scope: PolicyScope, scopeRef?: number | null) =>
    api
      .delete<void>(`/hr/schedule-policies/${scope}`, {
        params: scopeRef != null ? { scopeRef } : {},
      })
      .then((r) => r.data),
  ...
  myHistory: (params: { page?: number; size?: number } = {}) =>
    api
      .get<{
        ...
        size: number
        number: number
      }>("/hr/schedule-policies/me/history", {
        params: { page: 0, size: 20, ...params },
      })
      .then((r) => r.data),
}
